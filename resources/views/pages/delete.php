<?php
    ...
    if($connect) {
        ...
        if(mysqli_num_rows($use) != 0) {
           ...
           if(isset($_POST['tak'])) {
                $id = $_GET['Id_produktu'];
                $sql = "DELETE FROM produkty USING produkty WHERE Id_produktu = '$id'";
                $sqlimg = "DELETE FROM produkty_zdjęcia USING produkty_zdjęcia WHERE Id_produktu = '$id'";
                mysqli_query($connect, $sqlimg);
